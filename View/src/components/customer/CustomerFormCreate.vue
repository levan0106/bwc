...
export default {
    ...
    computed:{
        form(){
            return this.$store.getters['customer/info']
        },
    },
    created(){  
        //console.log(rules.Required)      
        this.$store.dispatch('customer/clearInfo')
        if(this.id >0){
            this.$store.dispatch('customer/pullInfo',this.id) 
            .then(()=>{                
                this.loading=false;
            })       
        }else{
        ...
        saveData: function(e){
            ...
            .then(_=>{
                if(_.valid){
                    //save data
                    if(this.id > 0){
                        this.$store.dispatch('customer/update',{id:this.id,data:this.form})
                        .then(_=>{
                            window.location.href = window.bwc.rootUrl + "/customer/list";
                        })  
                    }else{
                        this.$store.dispatch('customer/insert',this.form)
                        .then(_=>{
                            window.location.href = window.bwc.rootUrl + "/customer/list";
                        })
                    }
