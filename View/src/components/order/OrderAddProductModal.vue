...
export default {
    ...
    props:['open','data','orderId','id','order'],
    data(){
        return({
            loading:false,
            processing:false,
        ...
        computed:{
            ...
            GST(){
                return this.AMTExcGST * this.order.Taxes/100
            },
            AMTIncGST(){
            ...
            ReceivedGST(){
                return this.ReceivedAMTExcGST * this.order.Taxes/100
            },
            ReceivedAMTIncGST(){
        ...
        watch:{
            id:function (val){
                ...
                if(val > 0){
                    //get product
                    this.$store.dispatch('order/pullProduct',val)
                    .then(_=>{
                        this.loading=false
                        this.form = this.$store.getters['order/product']
        ...
        methods:{
            ...
            onProductChange(id){
                ...
                const discountDefault = discountList.filter(function(item){
                    return item.ObjectId == self.order.CustomerId && item.ProductId == self.form.ProductId
                })
           ...
           async productChange(id){                
                ...
                await this.$store.dispatch('product/pullAllPrice',id)
            },
            clearProductInfo(){
            ...
            updateData(){
                ...
                if(_.valid){
                        ...
                        self.$store.dispatch('order/updateProduct',{id:self.id,data:self.form})
                        .then(resolve=>{
                            this.$emit('save-data');
                            self.processing=false
                        })
